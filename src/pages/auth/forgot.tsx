...
export const ForgotPasswordPage = () => {
  ...
  const goToOtp = useCallback(() => {
    ...
    sendOtp(formData, {
      onSuccess: (res) => {
        if (res?.result) {
          const otpSent = getTranslation("otp_sent_to") || "OTP sent to";
          const checkPhone =
            getTranslation("please_check_your_phone") ||
          ...
          toast.error(
            res?.message ||
              getTranslation("user_is_not_found") ||
              "User is not found!"
          );
        }
      },
  ...
  }, [isPhoneValid, phone, sendOtp, getTranslation]);
    ...
    verifyCode(formData, {
      onSuccess: (res) => {
        if (res?.result) {
          toast.success(
            res?.message ||
              getTranslation("otp_verified_successfully") ||
          ...
          toast.error(
            res?.message ||
              getTranslation("otp_verification_failed") ||
    ...
    resetPassword(formData, {
      onSuccess: (res) => {
        if (res?.result && res?.access_token) {
          ...
          navigate("/");
          toast.success(
            getTranslation("password_reset_successful") ||
              "Password reset successful"
          );
        } else {
          toast.error(
            res?.message ||
              getTranslation("password_reset_failed") ||
  ...
  }, [canSubmitReset, password, resetPassword, getTranslation, navigate]);
  ...
  const titleText =
    step === "phone"
      ? getTranslation("forgot_password") || "Forgot Password?"
      : step === "otp"
      ? getTranslation("verify_code") || "Verify Code"
      : getTranslation("reset_password") || "Reset Password";

  const descriptionText =
    step === "phone"
      ? getTranslation("enter_your_phone_number_to_reset_your_password") ||
        "Enter your phone number to reset your password"
      : step === "otp"
      ? getTranslation("enter_the_6digit_code_sent_to_your_phone_number") ||
        "Enter the 6-digit code sent to your phone number"
      : getTranslation("set_a_new_password_for_your_account") ||
        "Set a new password for your account";

  return (
    <>
      <SeoWrapper
        title={getTranslation("forgot_password") || "Forgot password"}
      />
      <div
        style={{
                    ...
                    {step === "phone" && (
                      <motion.div
                        ...
                        <Button
                          ...
                          {getTranslation("continue") || "Continue"}
                        </Button>
                      </motion.div>
                    )}
                        ...
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <Password
                              ...
                              name="new-password"
                              label={
                                getTranslation("new_password") || "New Password"
                              }
                              className="md:h-12"
                              placeholder={
